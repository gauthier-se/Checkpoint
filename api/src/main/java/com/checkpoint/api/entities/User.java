...
public class User {
    ...
    // Relationship: User can have multiple games in their library
    @OneToMany(mappedBy = "user", cascade = CascadeType.ALL, orphanRemoval = true)
    private Set<UserGame> userGames = new HashSet<>();

    // Relationship: User can have multiple personal tags
    @OneToMany(mappedBy = "user", cascade = CascadeType.ALL, orphanRemoval = true)
    ...
    public User(String pseudo, String email, AuthProvider provider, String providerId) {
        this.pseudo = pseudo;
        this.email = email;
